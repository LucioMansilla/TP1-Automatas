...
Automata create_automaton(List *k, List *alphabet, List **delta,
                          int initial_state, List *final_states) {
  Automata a;
  a.k = k;
  a.alphabet = alphabet;
  ...
  for (int i = 0; i < automaton.k->size; i++) {
    for (int j = 0; j < ALPHABET_SIZE; j++) {
      for (int k = 0; k < automaton.delta[i][j].size; k++) {
        add(&result.delta[i + shift][j],
            mapping[get_data(&automaton.delta[i][j], k)]);
      }
    }
  }
  ...
  for (int i = 0; i < automaton.k->size; i++) {
    if (automaton.delta[i][LAMBDA].size) {
      for (int k = 0; k < automaton.delta[i][LAMBDA].size; k++) {
        add(&result.delta[i + shift][LAMBDA],
            mapping[get_data(&automaton.delta[i][LAMBDA], k)]);
      }
    }
  }

  Automata a = create_automaton(new_k, result.alphabet, result.delta,
                                result.initial_state, result.final_states);
  return a;
}
...
Automata kleene_clousure(Automata automaton) {
  ...
  for (int i = 0; i < shifted_automaton.final_states->size; i++) {
    add(&new_delta[get_data(shifted_automaton.final_states, i)][LAMBDA],
        shifted_automaton.initial_state);
    add(&new_delta[get_data(shifted_automaton.final_states, i)][LAMBDA],
        shifted_automaton.k->size - 1);
  }
...
Automata compose(Automata automaton_1, Automata automaton_2) {
  ...
  List *new_alphabet =
      union_alphabet(automaton_1.alphabet, automaton_2.alphabet);
  ...
  Automata shifted_automaton_2 =
      rename_states(automaton_2, automaton_1.k->size + 1, new_k);
  ...
  for (int i = 0; i < automaton_1.final_states->size; i++) {
    add(&new_delta[get_data(shifted_automaton_1.final_states, i)][LAMBDA],
        new_k->size - 1);
  }

  for (int i = 0; i < shifted_automaton_2.final_states->size; i++) {
    add(&new_delta[get_data(shifted_automaton_2.final_states, i)][LAMBDA],
        new_k->size - 1);
  }
...
Automata concat(Automata automaton_1, Automata automaton_2) {
  ...
  Automata automata_temp =
      rename_states(automaton_2, automaton_1.k->size, new_k);
  int shift = automaton_1.k->size;
  List *final_states = automata_temp.final_states;
  List **new_delta =
      (List **)malloc(sizeof(List) * (automaton_1.k->size + shift));
  ...
  for (int i = 0; i < automaton_1.final_states->size; i++) {
    add(&new_delta[get_data(automaton_1.final_states, i)][LAMBDA],
        automata_temp.initial_state);
  }
  Automata new = create_automaton(new_k, new_alphabet, new_delta,
                                  new_initial_state, final_states);
  print_automaton(new);
...
Automata minimize(Automata automaton) {
  ...
  int initial_state =
      get_equivalent_index(equiv_class, automaton.initial_state);
  ...
  for (int i = 0; i < size; i++) {
    ...
    for (int j = 0; j < automaton.alphabet->size; j++) {
      ...
      int to = get_equivalent_index(
          equiv_class,
          get_data(&automaton.delta[get_data(curr->list, 0)][symbol], 0));
      Transition temp =
  ...
  return create_automaton(K, automaton.alphabet, delta, initial_state,
                          final_states);
}
