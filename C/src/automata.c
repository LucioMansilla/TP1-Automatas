...
Automata compose(Automata automaton_1, Automata automaton_2) {
    ...
    if (automaton_1.k->size > automaton_2.k->size) {
        return compose(automaton_2, automaton_1);
    }
    int i = 0;
    ...
    for (int i = shift; i < new_k->size - 1; i++) {
        for (int j = 0; j < ALPHABET_SIZE; j++) {
...
Automata concat(Automata automaton_1, Automata automaton_2) {
    ...
    List *new_alphabet = copy_list(union_alphabet(automaton_1.alphabet, automaton_2.alphabet));
    ...
    for (int i = shift; i < new_k->size; i++) {
        for (int j = 0; j < ALPHABET_SIZE; j++) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        for (int j = 0; j < automaton.alphabet->size; j++) {
            ...
            if (transitions_count == max_transitions) {
                max_transitions *= 2;
                transitions = (Transition *)realloc(transitions, max_transitions * sizeof(Transition));
            }
...
Automata to_AFD(Automata a) {
    ...
    Transition *new_transitions = (Transition *)malloc(max_transitions * sizeof(Transition));
    for (int t_index = 0; t_index < t_size; t_index++) {
        ...
        while (j < alphabet_size) {
            ...
            Transition temp = create_transition(t_index, symbol, create_from((int[]){u_index}, 1));
            if (delta_count == max_transitions) {
                max_transitions *= 2;
                new_transitions = (Transition *)realloc(new_transitions, max_transitions * sizeof(Transition));
            }
...
void parser_transitions(int from, int to, int *symbols, int size_symbols, Transition *transitions,
    ...
    for (int i = 0; i < size_symbols; i++) {
        ...
        if (!already_symbol_exists) {
            transitions[*transition_count] = create_transition(from, symbol, create_from((int[]){to}, 1));
            (*transition_count)++;
        }
    }
}
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
        ...
        if (strstr(line, "->") != NULL && strstr(line, "inic->") == NULL) {
            ...
            while (symbol != NULL) {
                ...
                if (symbols_int[i] != LAMBDA) append(alphabet, symbols_int[i]);
                symbol = strtok(NULL, ",");
                i++;
            }
            parser_transitions(from, to, symbols_int, size, transitions, &transitions_count);
        }
    }
    transitions[transitions_count] = create_transition(-1, -1, NULL);
